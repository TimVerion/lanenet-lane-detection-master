...
"""
处理tusimple数据集脚本,生成gt_binary_image,gt_instance_image,train.txt,test.txt
"""
import argparse
import glob
import json
...
def process_json_file(json_file_path, src_dir, ori_dst_dir, binary_dst_dir, instance_dst_dir):
    ...
    with open(json_file_path, 'r') as file:
        for line_index, line in enumerate(file):
            ...
            src_image = cv2.imread(image_path, cv2.IMREAD_COLOR)# 读取原图
            dst_binary_image = np.zeros([src_image.shape[0], src_image.shape[1]], np.uint8)# 构建二进制mask图片
            dst_instance_image = np.zeros([src_image.shape[0], src_image.shape[1]], np.uint8)# 构建实例图片
